...
var SASLAuthenticator = require('./SASLAuthenticator.js');
...
ConnectionManager.prototype.onSocketConnected = function () {
    ...
    if (this.options.saslClient) {
        this.saslAuth = new SASLAuthenticator(this.options.saslClient);

        this.saslAuth.on('request', this.onSaslRequest.bind(this));
        this.saslAuth.on('error', this.onSaslError.bind(this));
        this.saslAuth.on('complete', this.onSaslComplete.bind(this));

        // Started after Connect response.
    }

    // "Traditional" auth and watches are queued here, but actually
    // delayed until SASL completion if an authenticator has been
    // configured.
...
ConnectionManager.prototype.onSocketData = function (buffer) {
    ...
    if (self.state === STATES.CONNECTING) {
        ...
        if (connectResponse.timeOut <= 0) {
            ...
            if (self.saslAuth) {
                self.saslAuth.start(self.socket);
            }
        ...
        default:
            ...
            if (responseHeader.err === 0) {
                ...
                case jute.OP_CODES.MULTI:
                    ...
                    break;
                case jute.OP_CODES.SASL:
                    responsePayload = new jute.protocol.SetSASLResponse();
                    break;
                default:
...
ConnectionManager.prototype.onPacketQueueReadable = function () {
    ...
    while ((packet = this.packetQueue.shift()) !== undefined) {
        ...
        if (this.saslAuth &&
            (header == null || header.type !== jute.OP_CODES.SASL)) {
            // Only process SASL packets during SASL authentication.
            // KLUDGE: Must unshift without generating an event!
            this.packetQueue.queue.unshift(packet);
            break;
        }
...
ConnectionManager.prototype.onSaslRequest = function (request, callback) {
    switch (this.state) {
    case STATES.CONNECTED:
    case STATES.CONNECTED_READ_ONLY:
        // SASL packets jump the queue.
        ...
        break;
    default:
        // Unexpected state; assume we lost the connection.
        callback(Exception.create(Exception.CONNECTION_LOSS));
        break;
    }
};
...
ConnectionManager.prototype.onSaslError = function (phase, error) {
    // console.log({l: 'SASL error', phase, error});
...
ConnectionManager.prototype.onSaslComplete = function () {
    this.saslAuth = null;

    // Check if we have anything to send out (delayed packets which
    // will now be send as the authenticator has been removed).
    this.onPacketQueueReadable();

    // For some reason, the Java client does not have a proper
    // "AUTHENTICATED" state; we follow the same "tradition."
