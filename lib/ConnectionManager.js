...
ConnectionManager.prototype.onSocketConnected = function () {
    ...
    var saslClient = this.options.saslClient;

    if (!saslClient) {
        this._setAuthAndWatches();
        return;
    }

    this.inSaslAuth = true;

    function setAuthFailed(error) {
    ...
    function setAuthenticated() {
        ...
        self._setAuthAndWatches();
    }

    function makeSaslRequest(token) {
    ...
    function onSaslStart(error, mech, token) {
        if (error) {
            setAuthFailed('SASL client start: ' + error);
            return;
        }

        var hasLastPacket = mech === 'GSSAPI';
        var isLastPacket = false;

        function onSaslContinue(error, isComplete, token) {
            if (error) {
                setAuthFailed('SASL client step: ' + error);
                return;
            }
            ...
            self.queue(makeSaslRequest(token), onSaslResponse);
        }

        function onSaslResponse(error, response) {
            if (error) {
                setAuthFailed('SASL response: ' + error);
                return;
            }
            ...
            saslClient.step(response.payload.token, onSaslContinue);
        }

        self.queue(makeSaslRequest(token), onSaslResponse);
    }

    saslClient.start(self.socket, onSaslStart);
};

ConnectionManager.prototype._setAuthAndWatches = function () {
    // Set auth info
