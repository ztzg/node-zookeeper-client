...
var sasl = undefined;
var sasl = require('node-sasl-wrapper');
...
ConnectionManager.prototype.onSocketConnected = function () {
    ...
    var client = new sasl.SaslWrapper();

    var connectResult = client.connect({
        service: 'zookeeper',
        // serverFQDN: 'teek.crosstwine.com',
    ...
    var clientStartResult = client.clientStart({
        mechlist: 'DIGEST-MD5'
    });
    ...
    payload.token = clientStartResult.clientout;

    var request = new jute.Request(header, payload);

    self.queue(request, function (error, response) {
        var clientStepResult = client.clientStep({
            serverin: response.payload.token.toString()
        });
        ...
        payload.token = clientStepResult.clientout;
